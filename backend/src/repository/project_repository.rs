...
impl ProjectRepository {
    pub async fn list_for_user(pool: &PgPool, user_id: Uuid) -> Result<Vec<ProjectWithRole>, sqlx::Error> {
        let projects = sqlx::query_as!(
            ...
            SELECT 
                ...
                p.is_private, p.shooting_technique_id, p.created_at, p.updated_at,
    ...
    pub async fn create(pool: &PgPool, user_id: Uuid, input: CreateProjectInput) -> Result<Project, sqlx::Error> {
        ...
        let project = sqlx::query_as!(
            Project,
            r#"
            INSERT INTO projects (title, slug, description, p_type, is_private, shooting_technique_id)
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING id, title, slug, description, p_type as "p_type: ProjectType", is_private, shooting_technique_id, created_at, updated_at
            "#,
            input.title,
            slug,
            input.description,
    ...
    pub async fn update(pool: &PgPool, project_id: Uuid, input: UpdateProjectInput) -> Result<Project, sqlx::Error> {
        ...
        let project = sqlx::query_as!(
            ...
            SET 
                ...
                shooting_technique_id = COALESCE($7, shooting_technique_id),
                updated_at = NOW()
            WHERE id = $1
            RETURNING id, title, slug, description, p_type as "p_type: ProjectType", is_private, shooting_technique_id, created_at, updated_at
            "#,
